...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        output = (x / torch.sqrt(torch.sum(x ** 2, dim = -1, keepdim = True) / x.size()[-1] + self.eps)) * self.weight.unsqueeze(0).unsqueeze(0)
        return output
...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config, debug = False):
        ...
        self.dropout_rate = config.attn_pdrop
        self.resid_dropout = nn.Dropout(config.resid_pdrop)
        # causal mask to ensure that attention is only applied to the left in the input sequence
        self.register_buffer("original_mask", (torch.tril(torch.ones(config.block_size, config.block_size))))
    ...
    def apply_rotary_emb(self, xq: torch.Tensor, xk: torch.Tensor, T: int) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        b, num_heads, T, head_dim = xq.size()
        seq_pos = torch.arange(T, device = device, dtype = torch.float)  # Shape: (T)
        self.inv_freq = self.inv_freq.to(device)
        freqs = torch.unsqueeze(self.inv_freq, 0).expand(T, head_dim//2)    # Shape: (T, dim // 2)
        pos_emb = ...  # Shape: (1, 1, T, dim)

        'What is pos_emb??'
        
        # Split pos into sin and cos components, repeating each to match xq and xk dimensions
        theta = torch.unsqueeze(seq_pos, 1).expand(T, head_dim//2) * freqs   # Shape:(T, dim//2)
        pos_sin = torch.sin(theta).unsqueeze(0).unsqueeze(0).unsqueeze(-1)  # Shape: (1, 1, T, head_dim//2, 1)
        pos_cos = torch.cos(theta).unsqueeze(0).unsqueeze(0).unsqueeze(-1)
        
        
        # Apply RoPE transformation: pair and rotate dimensions
        # Rotate query and key tensors
        rotator_1 = torch.cat([pos_cos, -pos_sin], dim = -1).unsqueeze(-1) # Shape: (1, 1, T, head_dim//2, 2, 1)
        rotator_2 = torch.cat([pos_sin, pos_cos], dim = -1).unsqueeze(-1) # Shape: (1, 1, T, head_dim//2, 2, 1)
        rotator = torch.cat([rotator_1,rotator_2], dim = -1).expand(b, num_heads, T, head_dim//2, 2, 2)  # Shape: (1, 1, T, head_dim//2, 2, 2)
        rotator_transformed = rotator.reshape((b*num_heads*T*head_dim // 2, 2, 2))

        xq_transformed = xq.reshape((b*num_heads*T*head_dim // 2, 2, 1))
        xk_transformed = xk.reshape((b*num_heads*T*head_dim // 2, 2, 1))

        xq_rot = torch.bmm(rotator_transformed, xq_transformed).reshape((b, num_heads, T, head_dim))
        xk_rot = torch.bmm(rotator_transformed, xk_transformed).reshape((b, num_heads, T, head_dim))

        'Is the reshaping a good vectorized way of multiplying?'
    ...
    def forward(self, x):
        ...
        qkv = (self.c_attn(x)).view((B, T, self.n_head, 3*self.head_dim)).permute(0, 2, 1, 3)
        ...
        if self.use_flash_attn:
            y = F.scaled_dot_product_attention(q, k, v, attn_mask = self.original_mask[:q.size(2),:q.size(2)], dropout_p = self.dropout_rate)
        else:
            # Compute attention scores
            att = torch.bmm(q.reshape((B * self.n_head, T, self.head_dim)), k.reshape((B * self.n_head, T, self.head_dim)).permute(0, 2, 1)) / math.sqrt(self.head_dim)
            att = att.reshape((B, self.n_head, T, T))

            # Apply causal mask
            self.mask = torch.tril((torch.ones(self.config.block_size, self.config.block_size)).T - torch.eye(self.config.block_size)).view(1, 1, self.config.block_size, self.config.block_size)
            ...
            y = torch.bmm(att.reshape((B * self.n_head, T, T)), v.reshape((B * self.n_head, T, self.head_dim))).reshape((B, self.n_head, T, self.head_dim))
...
class TransformerDecoderBlock(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mlpf = nn.Sequential(nn.Linear(config.n_embd, 4*config.n_embd), BERTGELU(), nn.Linear(4*config.n_embd, config.n_embd))
        self.residual_dropout = nn.Dropout(config.resid_pdrop)
        
    def forward(self, x):
        # Forward pass through the Decoder Layer
        out = self.mlpf(self.layer_norm_2(self.attention(self.layer_norm_1(x))))
        out = x + self.residual_dropout(out)
        return out # shape (b, T, n_embd)
...
class GPT(nn.Module):
    ...
    def forward(self, idx: torch.Tensor):
        ...
        for decoder in self.transformer.h:
            x = decoder(x)
        # Apply final layer normalization and linear layer to produce logits
        logits = self.lm_head(self.transformer.ln_f(x)) # shape: (b, t, n_embd)

        return logits # shape (b, T, n_embd)
    ...
    def generate(self, idx: torch.LongTensor, max_new_tokens: int, temperature:float = 1.0, do_sample:bool = False, top_k:int = None, top_p: float = 0.6):
        ...
        for _ in range(max_new_tokens):
            ...
            logit_final = logit[:,-1,:]
            logit_final /= temperature
            b, T, n_embd = logit.size()

            if not do_sample:
                # take the most likely token
                idx_next_indice = torch.argmax(logit_final, dim = -1)
            
            else:
                # apply softmax to convert logits to (normalized) probabilities
                softmax = nn.Softmax(dim = -1)
                logit_final = softmax(logit_final)

                # optionally only consider top-k logits for sampling. 
                if top_k is not None:
                    possible = torch.argsort(logit_final, dim = -1, descending= True)[:,:top_k] # shape: b * top_k
                    possibility = torch.ones((b * 1, top_k), device=device) # shape: b * top_k
                    idx_next_indice = torch.multinomial(possibility, num_samples=1) # shape: b * 1
                    idx_next = torch.gather(possible, -1, idx_next_indice) # shape: b * 1
                    

                # optionally apply top-p sampling
                if top_p is not None:

                    logit_sorted = logit_final.sort(dim = -1, descending= True).values
                    cumulative_logit = logit_sorted.cumsum(dim=-1)

                    # Design the mask
                    cutoff_mask = cumulative_logit >= top_p

                    # There is at least one word sampled
                    cutoff_mask[:,0] = 1

                    possibility = logit_sorted * cutoff_mask
                    possibility /= possibility.sum(dim=-1, keepdim=True) + 1e-10 # renormalize the prob
                    

                    idx_next_indice = torch.multinomial(possibility.view((b , n_embd)), num_samples=1).view((b, 1))
                    idx_sorted = torch.argsort(logit_final, dim = -1, descending=True)
                    idx_next = torch.gather(idx_sorted, -1, idx_next_indice)
